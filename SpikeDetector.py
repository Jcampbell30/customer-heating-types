import statistics
'''
@Revised By: Jared Campbell
@Date: 09/26/2023
@Description: This is a method for detecting spikes (or dips) within a dataset.
A data set should be an array where each entry consist of an item like: [date,usage] or [date,temp]
'''
def getSpikes(dataset,per):
    ...
    if per < 0: #if the per value is negative, and we are looking for dips
        for element in dataset:
